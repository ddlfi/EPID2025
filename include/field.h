...
class GF2_128 {
   ...
   public:
    ...
    inline void set_coeff(size_t idx) {
    ...
    GF2_128 multiply_with_GF2_matrix(
...
class GF2_192 {
   ...
   public:
    ...
    GF2_192 multiply_with_GF2_matrix(
...
class GF2_256 {
   ...
   public:
    ...
    GF2_256 multiply_with_GF2_matrix(
        const std::array<uint64_t, 4> *matrix) const;
...
std::vector<GF> get_first_n_field_elements(size_t n);
template <typename GF>
std::vector<std::vector<GF>> precompute_lagrange_polynomials(
...
template <typename GF>
GF eval(const std::vector<GF> &poly, const GF &point);
}  // namespace field
