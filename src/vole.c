...
// static
    ...
    // (depth + 1) x num_instances array of outLenBytes; but we only need to
    // rows at a time
    uint8_t* r = calloc(2 * num_instances, outLenBytes);

#define R(row, column) \
    (r + (((row) % 2) * num_instances + (column)) * outLenBytes)
#define V(idx) (v + (idx) * outLenBytes)
    ...
    for (unsigned int j = 0; j < depth; j++) {
        ...
        for (unsigned int i = 0; i < depthloop; i++) {
            ...
            xor_u8_array(R(j, 2 * i), R(j, 2 * i + 1), R(j + 1, i),
                         outLenBytes);
        }
    }
...
int ChalDec(const uint8_t* chal, unsigned int i, unsigned int k0,
            unsigned int t0, unsigned int k1, unsigned int t1,
            uint8_t* chalout) {
    if (i >= t0 + t1) {
        return 0;
    }
...
void vole_reconstruct(const uint8_t* iv, const uint8_t* chall,
    ...
    for (unsigned int i = 0; i < tau; i++) {
        ...
        // Step: 7..8
        ConvertToVole(iv, sd, true, lambda, depth, ellhat_bytes, NULL,
                      q[q_idx]);
        q_idx += depth;
