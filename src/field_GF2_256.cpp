...
GF2_256::GF2_256(std::string hex_string) {
    ...
    // mid_hi 64 bit
    uint64_t mid_hi =
        std::stoull(hex_string.substr(64 / 4, 64 / 4), nullptr, 16);
    // mid_lo 64 bit
    uint64_t mid_lo =
...
GF2_256 GF2_256::multiply_with_transposed_GF2_matrix(
    ...
    for (unsigned int w = 4; w; --w, ++vptr) {
        ...
        for (unsigned int i = sizeof(uint64_t) * 8; i;
            ...
            cval[0] = _mm256_xor_si256(
                cval[0],
                _mm256_and_si256(Ablock[0], mm256_compute_mask(idx, 0)));
            cval[1] = _mm256_xor_si256(
                cval[1],
                _mm256_and_si256(Ablock[1], mm256_compute_mask(idx, 1)));
            cval[0] = _mm256_xor_si256(
                cval[0],
                _mm256_and_si256(Ablock[2], mm256_compute_mask(idx, 2)));
            cval[1] = _mm256_xor_si256(
                cval[1],
                _mm256_and_si256(Ablock[3], mm256_compute_mask(idx, 3)));
        }
    }
    GF2_256 result;
...
field::GF2_256 combine_gf_vec(field::GF2_256 *gf_vec) {
    ...
    for (int i = 0; i < 32; i++) {
        for (int j = 7; j >= 0; j--) {
            result += gf_vec[8 * i + j] * field_base_256[8 * i + j];
        }
    }
    return result;
}